...
export default function Sidebar({ selectedTexts, setSelectedTexts }) {
  ...
  const handleSelectAvailabilities = (availability) => {
    ...
    setSelectedAvailabilities((prev) => {
      ...
      // Return the updated selected availabilities
      return newSelectedAvailabilities;
    });
  };

  useEffect(() => {
    // Calculate the new selected texts after `selectedAvailabilities` is updated
    const newSelectedTexts = selectedAvailabilities.map((el) => el.text);
    setSelectedTexts(newSelectedTexts);
  }, [selectedAvailabilities]);  // This will run every time `selectedAvailabilities` changes
                ...
                {availabilities.map((availability) => (
                  ...
                  >
                    <input
                      ...
                      className="tf-check"
                      readOnly
                      checked={selectedAvailabilities.some((item) => item.text === availability.text)}
                    />
