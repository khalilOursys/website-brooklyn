...
export default function ProductCard6({ product }) {
  ...
  return (
    <div
      className={`card-product style-price radius-20 ${product.populer ? "bg_black " : ""
        }`}
    >
      ...
      <div className="card-product-info">
        ...
        <ul className="list-color-product">
          {product.colors.map((color, idx) => (
            <li
              key={idx}
              className={`list-color-item color-swatch ${currentImage == color.image ? "active" : ""
                }  `}
      ...
      <div className="tf-price-table-contents">
        ...
        <div className="tf-price-table-btn text-center">
          <Link
            href={`/product-detail/${product.id}`}
            className={`tf-btn btn-md  rounded-full ${product.populer ? "btn-light-icon" : "btn-fill btn-icon"
              }  animate-hover-btn`}
          >
            View details
          </Link>
